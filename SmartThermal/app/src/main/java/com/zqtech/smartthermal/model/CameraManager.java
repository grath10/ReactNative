...
public class CameraManager {
    ...
    public synchronized void realPlay() {
        ...
        if(m_iPlayID < 0){
            ...
            ClientInfo.dwStreamType = 0; // 0-主码流，1-子码流，2-码流3，3-码流4，以此类推
            ClientInfo.bBlocked = 1; // 0-非阻塞取流，1-阻塞取流
    ...
    private void startSinglePreview() {
        ...
        if (fRealDataCallBack == null) {
            Log.e(TAG, "fRealDataCallBack object failed!");
            return;
        }
...
//         struClienInfo, fRealDataCallBack, false);
        if (m_iPlayID < 0) {
            Log.e(TAG, "NET_DVR_RealPlay failed! Err:"
                    + HCNetSDK.getInstance().NET_DVR_GetLastError());
            return;
        }

        Log.i(TAG,
                "NetSdk Play success ***********************3***************************");
...
//        }
    ...
    public synchronized void stopPlay() {
        ...
        if (HCNetSDK.getInstance().NET_DVR_StopRealPlay(m_iPlayID)) {
            ...
            Log.e(TAG,
                    "停止实时播放失败！"
                            + getErrorMsg(HCNetSDK.getInstance().NET_DVR_GetLastError()));
            return;
        }
        // 停止本地播放
